package gtl.index.strtree.impl;


import gtl.jts.geom.Envelope;
import gtl.jts.index.ItemVisitor;
import gtl.jts.index.strtree.*;
import gtl.jts.util.PriorityQueue;
...
import gtl.index.strtree.AbstractVisitor;
...
public class STRTreeImpl extends STRtree implements STRTree{
     ...
     */
    public synchronized  void insert(gtl.geom.Envelope itemEnv, Object item) {
     ...
     *           Object  -- insert(gtl.geom.Envelope itemEnv, Object item)
     *           Geometry -- insert(gtl.geom.Geometry)
     * @param searchEnv
     * @return
     */

    public List<Object> query(gtl.geom.Envelope searchEnv) {
        return super.query(JTSWrapper.toJTSEnvelope(searchEnv));
    }

    public void query(gtl.geom.Envelope searchEnv, AbstractVisitor visitor){
        super.query(JTSWrapper.toJTSEnvelope(searchEnv),(ItemVisitor)visitor);
    }

    public List<gtl.geom.Envelope> getLeafNodeEnvelopes() {
        List list = queryBoundary();
        if(list==null) return null;
        List<gtl.geom.Envelope> results =new ArrayList<>(list.size());
        for(Object o: list){
    ...
    public Object[] nearestNeighbour(gtl.geom.Envelope env, Object item, ItemDistance itemDist,int k)
    {
    ...
    protected Object[] nearestNeighbour(BoundablePair initBndPair, int k)
    {
        return nearestNeighbour(initBndPair, Double.POSITIVE_INFINITY,k);
    }

    protected Object[] nearestNeighbour(BoundablePair initBndPair, double maxDistance, int k)
    {
    /*
