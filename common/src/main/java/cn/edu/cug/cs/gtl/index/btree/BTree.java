...
import cn.edu.cug.cs.gtl.index.btree.unboxed.*;
 ...
 * not.<p>
 * <p>
 * ou can associate a <i>summary</i> with each leaf node of the BTree.
 * n order to do this, you must provide an instance of {@see
 * ssociativeOperation}
 * o the BTree when you construct it.  The AssociativeOperation knows
 * he key and value type of the BTree, and must know two things:
 *
 * <ul>
        ...
        while (true) {
            ...
            if (cur.isLeafNode()) {
                switch (op) {
                    ...
                    case GET_VAL_FROM_KEY_CEIL:  /* FIXME: might need to backtrack one step */
                        throw new RuntimeException("not implemented");
                    case GET_ORD_FROM_KEY:
                        return comp == 0 ? new Integer(idx + global_ord) : new Integer(-1);
                    case GET_ORD_FROM_KEY_FLOOR:
                        return new Integer(idx + global_ord /*FIXME: off the end?*/);
                    case GET_ORD_FROM_KEY_CEIL:
                ...
                if (op == Op.INSERT) {
                    if (cheat) insertionFastPath++;
                    else insertionSlowPath++;
                }
