...
public class UnboxedInt implements UnboxedComparable<Integer> {
    ...
    public Integer deserialize(byte[] buf, int ofs) {
        return new Integer(deserializeInt(buf, ofs));
    }

    public void serialize(Integer k, byte[] buf, int ofs) {
        serializeInt(k.intValue(), buf, ofs);
    }
    ...
    public int compare(byte[] buf1, int ofs1, byte[] buf2, int ofs2) {
        ...
        i = (buf1msb) - (buf2msb);
        if (i != 0) return i;
        i = (buf1[ofs1 + 2] & 0xff) - (buf2[ofs2 + 2] & 0xff);
        if (i != 0) return i;
        i = (buf1[ofs1 + 1] & 0xff) - (buf2[ofs2 + 1] & 0xff);
        if (i != 0) return i;
        i = (buf1[ofs1 + 0] & 0xff) - (buf2[ofs2 + 0] & 0xff);
        if (i != 0) return i;
        return 0;
    }
}
