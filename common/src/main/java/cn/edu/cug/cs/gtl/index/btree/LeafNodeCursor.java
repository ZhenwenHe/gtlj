...
class LeafNodeCursor
    ...
    public static boolean isLeafNode(CachingPageStorage.CachedPage cp) {
        return UnboxedInt.instance.deserializeInt(cp.getBuf(), 1 * SIZEOF_INT) == 0;
    }
    ...
    public int getLeftNeighborPageId() {
        return bt.ui.deserializeInt(getBuf(), 2 * SIZEOF_INT);
    }

    public int getRightNeighborPageId() {
        return bt.ui.deserializeInt(getBuf(), 3 * SIZEOF_INT);
    }

    protected void setNumBuckets(int num) {
        bt.ui.serializeInt(numbuckets = num, getBuf(), 4 * SIZEOF_INT);
    }
    ...
    protected int endOfBuf() {
        return LEAF_HEADER_SIZE + LEAF_ENTRY_SIZE * getNumBuckets();
    }
    ...
    public K getKey(int bucket) {
        return bt.uk.deserialize(getBuf(), LEAF_HEADER_SIZE + LEAF_ENTRY_SIZE * bucket);
    }

    public int getNumValsBelowBucket(int bucket) {
        return bucket < getNumBuckets() ? 1 : 0;
    }
