...
public class DefaultDataSet<S,L> implements DataSet<S,L>, Storable {
    ...
    public boolean load(DataInput dataInput) throws IOException {
        ...
        if(s>0){
            ...
            for(int i=0;i<s;++i)
                this.samples.add((S)gtl.util.ObjectUtils.load(dataInput));
        }
        s = dataInput.readInt();
        if(s>0){
            this.labels=new ArrayList<>(s);
            for(int i=0;i<s;++i)
                this.labels.add((L)gtl.util.ObjectUtils.load(dataInput));
        }
        return true;
    }
    ...
    public boolean store(DataOutput dataOutput) throws IOException {
        ...
        if(s>0){
            for(S si : this.samples)
                gtl.util.ObjectUtils.store(si,dataOutput);
        }
        ...
        if(s>0){
            ...
            for(L li : this.labels)
                gtl.util.ObjectUtils.store(li,dataOutput);
        }
        return true;
    }
